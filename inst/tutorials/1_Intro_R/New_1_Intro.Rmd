...
R is a powerful, free and open-source program for statistical computing. It is 
fast becoming the standard software for data analysis in the social sciences,
particularly in political science. Some of its key advantages are:

* It is extremely flexible, almost anything can be done with it.
* It is free.
* It is constantly improving. R code now is much more accessible, faster, and 
powerful than previously.
* Once you master the basics, it is just a matter of adjusting that code to new data.

These basics are what this course is about. It will introduce you to everything
you need to know to use R. Once you complete this course, you can build on what 
you've learned.
...
We first dive into operators and consider functions later.
...
4. Division: `/`
5. Exponentiation: `^`
6. Modulo (Division with remainder): `%`
...
An assignment operator is a special type of verb. It is akin to the verb 
**to be**. There are two of them that work in almost the same way:

* `<-`
* `=`

Both are used to assign a name to a value or a set of values. However, in practice you should **almost always** use `<-`.

With the following code, we tell R that we want to save the number 1791 under the name `start_of_haitian_revolution`:

```r
start_of_haitian_revolution <- 1791
```

Here, 1791 refers to the beginning of the Haitian Revolution, when enslaved people and other inhabitants of the French colony of Saint-Domingue rose up against French colonial rule and slavery. The revolution ultimately led to the establishment of the independent state of Haiti in 1804.

The name and the number are stored together in the R environment, which can be understood as the memory you are working with during an R session. Named entities are generally referred to as **objects** in R (more on this later).

Once you have created an object, you can print its value on the screen by typing `start_of_haitian_revolution` in the R console.
...
Here is an example. You can run the code and then type `start_of_haitian_revolution` in the 
...
```{r start-HT, exercise=TRUE, exercise.lines = 4}
start_of_haitian_revolution <- 1791

start_of_haitian_revolution
...
```{r end-HT, exercise=TRUE, exercise.setup = "start-HT", exercise.lines = 3}
end_of_haitian_revolution <- 1804
```
...
With the objects `start_of_haitian_revolution` and `end_of_haitian_revolution`, you can now calculate the 
length of the haitian revolution *without* having to create a new object using an 
assignment operator (just as with an ordinary calculator). 

```{r assign_diff, exercise = TRUE, exercise.setup = "end-HT", exercise.lines = 7}
start_of_haitian_revolution <- 1791
end_of_haitian_revolution <- 1804

end_of_haitian_revolution-start_of_haitian_revolution
```
...
```{r object_w_blanks, exercise = TRUE, exercise.setup = "end-HT", exercise.lines = 8}
start_of_haitian_revolution <- 1791
end_of_haitian_revolution <- 1804

`duration of Haitian Revolution` <- end_of_haitian_revolution-start_of_haitian_revolution

`duration of Haitian Revolution` 
```
...
because you need at least two numbers for building a sum. In the chunks above, we 
...
```{r default-digits, exercise = TRUE, exercise.lines = 3}
round(2.654)
...
## Topic 4: Grammar of R (Data Types, Vectors & Data Frames)
...
- **numbers** (also known as **numeric values**)
...
object `start_of_haitian_revolution`.
...
class(start_of_haitian_revolution)
...
as objects in R. Note in the code below that, unlike the numeric values,
one assigns textual data to an object with the help of quotation marks. 
Quotation marks are the syntax that makes it clear to R that what comes in them 
...
bigger_number <- large_number < low_number
...
named object. For example, in the code below we concatenate names of the federal states in Germany
that have federal elections in 2026 into an object 
called federal_elections_germany:

```{r string_vector, exercise=TRUE, exercise.lines = 5}
# Creating a string vector
federal_elections_germany <- c("Baden-Württemberg", "Rheinland-Pfalz", "Sachsen-Anhalt", "Berlin", "Mecklenburg-Vorpommern")

federal_elections_germany
...
elections_months <- c(3, 3, 9, 9, 9)

elections_months

```

Lastly, using the same function, we can create logical vectors. For example, let us indicate whether the selected German federal states have a **Landtag**. `TRUE` indicates that a federal state has a Landtag, while `FALSE` indicates that it does not.
  

```{r logical_vector, exercise=TRUE, exercise.lines = 5}
# Creating a logical vector
is_landtag <- c(
  TRUE,   # Baden-Württemberg
  TRUE,   # Rheinland-Pfalz
  TRUE,   # Sachsen-Anhalt
  FALSE,  # Berlin
  TRUE    # Mecklenburg-Vorpommern
)
...
**Naming Vectors**
We can also name vectors in R. When naming a vector, we assign a ## Naming Vectors

We can also name vectors in R. When naming a vector, we assign a name to each element of the vector. This can be done with the `names()` function.

In the following example, we use the `federal_states` vector to name the elements of the `is_landtag` vector. This allows us to see which value belongs to which federal state.


```{r naming_vectors, exercise=TRUE, exercise.lines = 11}
# Vector of Bundesländer that held elections
federal_states <- c(
  "Baden-Württemberg",
  "Rheinland-Pfalz",
  "Sachsen-Anhalt",
...
# Vector indicating whether the election was for the Landtag or Abgeordnetenhaus
is_landtag <- c(TRUE, TRUE, TRUE, FALSE, TRUE)

# Naming the is_landtag vektor
names(is_landtag) <- federal_states
is_landtag
```

As you can see in the printout, the vector `is_landtag` now has a federal state name for each of its values. This makes it easier to see which `TRUE` or `FALSE` value belongs to which federal state.
...
# Vector of German federal states
federal_states <- c(
  "Baden-Württemberg",
  "Rheinland-Pfalz",
  "Sachsen-Anhalt",
...
# Vector indicating whether the elections were for the Landtag or the Abgeordnetenhaus
is_landtag <- c(TRUE, TRUE, TRUE, FALSE, TRUE)

# Naming the is_landtag vector
names(is_landtag) <- federal_states

# Selecting the first value in a vector
is_landtag[1]

# Selecting the fourth value in a vector
is_landtag[4]
```

Another approach to selecting a value from a vector is to use its name. Similar to the example above, we use square brackets. But instead of using a numeric index, we specify the name of the vector value we want to select. Since names are string values, we need to put them in quotation marks.

For example, let's check whether the elections in **Baden-Württemberg** and **Berlin** were for the Landtag or the Abgeordnetenhaus:
...
# Selecting the value for Baden-Württemberg
is_landtag["Baden-Württemberg"]

# Selecting the value for Berlin
is_landtag["Berlin"]

```

## Topic 4.3: Data frames
...
representing different data types: numeric, character, logical. Data 
...
# Creating a data frames called elections
...
With `data()` you can load a data frame. It needs to be saved in the same folder as your R script.
...
with it. First of all, one can see how many rows and columns a data frame 
has with the `dim()`, `nrow()`, `ncol()` functions. One can also look up the names 
...
Remember that with `colnames()` you can have a look at the names of the variables. 
...
```{r df_rc365467, exercise=TRUE, exercise.lines = 13}
data("cabinet")
...
Once we can select values in data frames, adding information becomes easy. 
...
cabinet_added <- cbind(cabinet, fictional_vector)
head(cabinet_added)
...
Looks like it's seven. We therefore need to create a vector that has seven values.
This new vector is a fictional party in a fictional election. We then add that 
vector to the elections data frame.

```{r df_crn7667788990, eval = F, exercise=TRUE, exercise.lines = 11}
data("elections")
...
Packages are akin to books that are specific to a field. Packages consist of functions (verbs) that perform specific tasks. For instance, there are packages that provide functions for regression analysis, data visualization, or organizing and cleaning data. These field-specific packages are available from repositories such as [CRAN](https://cran.r-project.org/).

When using packages in R, we have to remember two important things. First, we have to install a package before we can use it. This is done with the special function `install.packages("package name")`. Notice that the name of the package is written in quotation marks. In the example below, the installation commands are commented out (the line starts with a `#`), which means that they will not be evaluated. This is because the packages have already been installed as part of the tutorial setup.

```{r pack_ins1, eval = F, exercise=TRUE, exercise.lines = 7}
# Example of installing a package
# install.packages("ggplot2") # ggplot2 is a package for data visualization

# Installing the gerda package
# install.packages("gerda")
```


The second important point is that installing a package and using a package are two different things. After a package has been installed, we still need to load it into our current R session. To do this, we use the `library()` function. Unlike `install.packages()`, we do not use quotation marks around the package name when using `library()`.

For example, the following code loads the packages we need for the example below:
...
The `gerda` package provides access to the German Election Database (GERDA), which contains harmonized and unharmonized election data for German elections. In this example, we use the unharmonized federal election data at the constituency level.

Before working with the data, we can use `gerda_data_list()` to see which datasets are available through the package:

```{r pack_ins3}
gerda_data_list()
```

We then use `load_gerda_web()` to load the federal election data. The name of the dataset is provided as a character string:
...
The resulting data frame contains information on the German federal elections at the constituency level. Among other variables, it includes the election year, constituency, whether the observation refers to the first or second vote, and the vote share of individual parties.

For our example, we are interested in the 2025 Bundestag election and in the second votes. We therefore use `filter()` to select these observations:
...
We can now summarize the data to calculate the mean second-vote share for each of the selected parties across constituencies. We then reshape the data from a wide format into a long format, which makes it easier to use with `ggplot2`.

You do **not** need to understand all of this code yet. We will learn how to summarize and reshape data in more detail later in the course. For now, the important point is to see how these steps can be combined to prepare data for a visualization.

```{r pack_ins6}
party_means <- DE_2025 %>% 
  summarise(
    `CDU/CSU` = mean(cdu_csu, na.rm = TRUE),
    SPD = mean(spd, na.rm = TRUE),
    AfD = mean(afd, na.rm = TRUE),
    GRÜNE = mean(gruene, na.rm = TRUE),
    LINKE = mean(linke_pds, na.rm = TRUE)
  ) %>% 
  pivot_longer(
    cols = everything(),
    names_to = "party",
...
We can now use this summary data to create a barplot with `ggplot2`. The bars show the mean second-vote share across constituencies for each party. The `fill` aesthetic allows us to assign a different color to each party, while `scale_fill_manual()` lets us specify the colors ourselves.
...
ggplot(
  ...
  labs(
    y = "Mean second-vote share",
    x = "Party",
    title = "2025 Bundestag Election",
    subtitle = "Mean second-vote share across constituencies"
  ) +
  theme_bw() +
  theme(
...
The following code is an example of what packages can do. **Please do not try to memorize the code.** There is a separate module on data visualization within your program that covers `ggplot2` and its functions in much more detail.

For now, the important point is to recognize how different packages can work together: `gerda` provides access to election data, `dplyr` helps us manipulate and summarize the data, `tidyr` helps us reshape the data, and `ggplot2` is used to create the visualization. In the following chapter, we will take a closer look at how dplyr works. 

## Topic 6: Tidy Data & Data manipulation
...
data("cabinet")
BG_subset <- select(cabinet,country_name, year, party_name_english, seats)
BG_subset <- filter(BG_subset,country_name == "Bulgaria")
head(BG_subset)
```
...
```{r dplyr9, exercise=TRUE, exercise.lines = 7}
data("cabinet")
...
```{r dplyr10, exercise=TRUE, exercise.lines = 7}
data("cabinet")
BG_subset <- filter(cabinet,country_name == "Bulgaria")
BG_subset <- select(BG_subset,country_name, year, party_name_english, seats)
...
select the variables: `party_name_english`, `year`, and `seats`. Name the
...
```{r dplyr13, exercise=TRUE, exercise.lines = 7}
data("cabinet")
# calculate the mean number of seats in our cabinet data
summarise(cabinet, mean(seats))
```

While this is useful, you might wonder why we need to use summarize in the 
...
```{r dplyr14, exercise=TRUE, exercise.lines = 7}
data("cabinet")
...
AU_subset <- cabinet[cabinet$country_name == "Australia",]

# Subset for Austria
AT_subset <- cabinet[cabinet$country_name == "Austria",]

# Subset for Belgium
BE_subset <- cabinet[cabinet$country_name == "Belgium",]
...
**7.5: mutate() for creating new values**
...
## Topic 7: Advanced tidy: the pipe operator
...
There are many more things to learn about R, such as lists, merging datasets (e.g. with left_join),
